...
echo KILL SCRIPT FOR AGENT CSNBM77Q
echo Current server PID: 14200 (will be protected)
echo Checking PID file...
if exist "C:\Users\User\Repnix\agent_tracking\agent_CSNBM77Q.pid" (
  for /f "tokens=*" %%p in ('type "C:\Users\User\Repnix\agent_tracking\agent_CSNBM77Q.pid"') do (
    echo Found PID: %%p
...
echo Killing processes with agent ID CSNBM77Q...
for /f "usebackq tokens=1" %%p in (`wmic process where "name='python.exe' and commandline like '%CSNBM77Q%' and not commandline like '%--port 8000%' and not commandline like '%--port 8001%'" get processid /format:value ^| find "ProcessId"`) do (
  set pline=%%p
  set pid=!pline:ProcessId=!
  if not "!pid!" == ""{current_pid}" (
...
echo Killing CMD windows with agent ID CSNBM77Q...
taskkill /F /FI "WINDOWTITLE eq *CSNBM77Q*" /T
if %errorlevel% equ 0 (
...
if exist "C:\Users\User\Repnix\agent_tracking\agent_CSNBM77Q.pid" del "C:\Users\User\Repnix\agent_tracking\agent_CSNBM77Q.pid" 2>nul
if exist "C:\Users\User\Repnix\agent_tracking\wrapper_CSNBM77Q.py" del "C:\Users\User\Repnix\agent_tracking\wrapper_CSNBM77Q.py" 2>nul
