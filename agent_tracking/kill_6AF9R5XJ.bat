...
echo KILL SCRIPT FOR AGENT 6AF9R5XJ
echo Current server PID: 15904 (will be protected)
echo Checking PID file...
if exist "C:\Users\User\Repnix\agent_tracking\agent_6AF9R5XJ.pid" (
  for /f "tokens=*" %%p in ('type "C:\Users\User\Repnix\agent_tracking\agent_6AF9R5XJ.pid"') do (
    echo Found PID: %%p
...
echo Killing processes with agent ID 6AF9R5XJ...
for /f "usebackq tokens=1" %%p in (`wmic process where "name='python.exe' and commandline like '%6AF9R5XJ%' and not commandline like '%--port 8000%' and not commandline like '%--port 8001%'" get processid /format:value ^| find "ProcessId"`) do (
  set pline=%%p
  set pid=!pline:ProcessId=!
  if not "!pid!" == ""{current_pid}" (
...
echo Killing processes with user ID bank...
for /f "usebackq tokens=1" %%p in (`wmic process where "name='python.exe' and commandline like '%bank%' and not commandline like '%--port 8000%' and not commandline like '%--port 8001%'" get processid /format:value ^| find "ProcessId"`) do (
  set pline=%%p
  set pid=!pline:ProcessId=!
  if not "!pid!" == ""{current_pid}" (
...
echo Killing CMD windows with agent ID 6AF9R5XJ...
taskkill /F /FI "WINDOWTITLE eq *6AF9R5XJ*" /T
if %errorlevel% equ 0 (
...
if exist "C:\Users\User\Repnix\agent_tracking\agent_6AF9R5XJ.pid" del "C:\Users\User\Repnix\agent_tracking\agent_6AF9R5XJ.pid" 2>nul
if exist "C:\Users\User\Repnix\agent_tracking\wrapper_6AF9R5XJ.py" del "C:\Users\User\Repnix\agent_tracking\wrapper_6AF9R5XJ.py" 2>nul
